//! Transport plumbing for the Dice gateway (server half) and, in a later
//! milestone-1 phase, the desktop client (client half).
//!
...
// mod client — arrives in a later milestone-1 phase behind feature "client":
// the desktop client's AnyTransport enum (QUIC/WSS), gateway driver, and
// reconnect/backoff. Deliberately absent until then.
