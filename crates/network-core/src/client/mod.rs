...
//! talk to a Dice gateway, WSS-only in this phase (docs/design/
//! desktop-client.md §1; QUIC slots into [`AnyTransport`] in Phase 4).
//!
//! - [`tls`](mod@tls): [`TlsOptions`] — webpki roots plus optional extra
//!   anchors (the dev CA via `DICE_DEV_CA`). Full verification always runs;
//!   there is deliberately NO verification-off switch.
//! - [`transport`]: [`WssTransport`] / [`AnyTransport`] — one binary WS
//!   message = one bare `dice.v1.Frame`.
//! - [`gateway`]: the driver task — Identify/Resume handshake, heartbeats,
//!   full-jitter backoff — behind [`GatewayHandle`].
...
pub use transport::{AnyTransport, ClientTransportError, WssTransport};
